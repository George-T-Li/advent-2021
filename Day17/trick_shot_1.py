...
def main():
    ...
    vx_min = 0
    while triangle(vx_min) < xr_min:
        vx_min += 1
    vx_max = vx_min
    while triangle(vx_max) < xr_max:
        vx_max += 1
    vx_range = list(range(vx_min, vx_max))
    h_list = [None for x in vx_range]

    for index, vx in enumerate(vx_range):
        vy = yr_min
        while True:
            landed = fire(vx, vy, x_range, y_range)
            if landed[0]:
                h_list[index] = landed[1]
            else:
                if vy > abs(yr_min):
                    break
            vy += 1
